...
import { getNextYortzite, renderPlainHebDate } from 'src/app/utils/hebrew-date.util';
...
export class DedicationFormComponent implements OnInit {
  ...
  constructor(
    ...
    this.dedicationForm = new FormGroup({
      ...
      title: new FormControl('', [Validators.required, Validators.pattern(/^[a-zA-Z0-9א-ת-'",. ]{2,}$/)]),
      description: new FormControl('', [Validators.required, Validators.pattern(/^[a-zA-Z0-9א-ת-'",. ]{2,}$/)]),
      isRepeated: new FormControl(false),
      date: new FormGroup({
        day: new FormControl('', [Validators.required, Validators.min(1), Validators.max(30)]),
        month: new FormControl('', [Validators.required, Validators.min(1), Validators.max(13)]),
        year: new FormControl('', [Validators.required, Validators.min(5500), Validators.max(6000)]),
      })
    });
    ...
    this.acRoute.params.subscribe(p => {
      ...
      if (typeof (dedication) != "undefined") {
        this.dedicationForm.setValue({
          ...
          date: { day: dedication.date.day, month: dedication.date.month, year: dedication.date.year },
  ...
  private expiredAt(isRepeated: boolean) {
    return isRepeated ? new Date("01/01/4000") : addDays(getNextYortzite(this.formDate.value), 7);
  }
